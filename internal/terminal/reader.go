...
func (t EscapeSequence) applyGraphicRenditionOn(s *Screen) error {
	code := mylog.Check2(t.ExpectNumber(0, 0))
	...
	if code == 0 {
		...
		// RGB Code
		n := mylog.Check2(t.ExpectNumber(1, 0))

		if n == 2 {
			// Gather the rest
			r := mylog.Check2(t.ExpectNumber(2, 0))
			g := mylog.Check2(t.ExpectNumber(3, 0))
			b := mylog.Check2(t.ExpectNumber(4, 0))
			...
			c := mylog.Check2(t.ExpectNumber(2, 0))
		...
		slog.Debug(fmt.Sprintf("Unsupported code: %d", code))
	}
...
func (t EscapeSequence) applyCursorPosition(s *Screen) error {
	...
	line := mylog.Check2(t.ExpectNumber(0, 1))

	column := mylog.Check2(t.ExpectNumber(1, 1))
...
func (r *AnsiReader) parseEscapeSequence() EscapeSequence {
	...
	for r.HasNext() {
		...
		if unicode.IsDigit(rune(b)) {
			...
			n := mylog.Check2(strconv.Atoi(buf.String()))
