...
func exeAndroid(tmpDir string, tools *androidTools, bi *buildInfo, extraJars, perms []string, isBundle bool) (err error) {
	...
	network_security_config, err := os.Create(filepath.Join(xmlDir, "network_security_config.xml"))
	if err != nil {
		return err
	}
	network_security_config.Write([]byte(`<?xml version="1.0" encoding="utf-8"?>
...
<application 
    ...
    {{.IconSnip}} android:label="{{.AppName}}">
