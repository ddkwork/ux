...
func ConvertInteger2DemoType[T constraints.Integer](v T) DemoType {
	return DemoType(v)
}

func (t DemoType) AssertType(name string) DemoType {
	for _, Type := range t.Types() {
		if strings.ToLower(name) == strings.ToLower(Type.String()) {
			return Type
		}
	}
	return InvalidDemoType
}
...
func (t DemoType) Types() []DemoType {
	return []DemoType{
		TreeTableType,
		TreeType,
		Table2Type,
