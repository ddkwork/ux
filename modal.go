...
func (m *ModalView) Anim() *cmp.VisibilityAnimation {
	return m.anim
}
...
func (m *ModalView) Layout(gtx layout.Context) layout.Dimensions {
	m.update(gtx)
	if !m.anim.Visible() {
...
func (m *ModalView) layoutView(gtx layout.Context) layout.Dimensions {
	if m.MaxWidth == 0 {
...
func (m *ModalView) layoutHeader(gtx layout.Context) layout.Dimensions {
	return layout.Flex{
		...
		layout.Rigid(func(gtx C) D {
			return material.Clickable(gtx, &m.closeBtn, func(gtx C) D {
				return NavigationCloseIcon.Layout(gtx, th.Fg)
			})
		}),
	)
}
